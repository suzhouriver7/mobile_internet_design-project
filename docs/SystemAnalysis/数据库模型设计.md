...
| **字段**         | **类型**         | **主外键** | **中文说明**                          |
|----------------| ---------------------- | ---------------- | ------------------------------------------- |
| **uid**        | **BIGINT**       | **主键**   | **用户唯一编号**                      |
| **email**      | **VARCHAR(100)** |                  | **邮箱（可用于登录与密码找回）**      |
| **password**   | **VARCHAR(255)** |                  | **加密后的密码，不能存明文**          |
...
| **字段**         | **类型**         | **主外键**             | **中文说明**                 |
|----------------| ---------------------- |---------------------| ---------------------------------- |
| **oid**        | **BIGINT**       | **主键**              | **订单唯一 ID**              |
| **uid**        | **BIGINT**       | **外键 → users(uid)** | **发布订单的用户**           |
...
| **字段**  | **类型**          | **主外键**           | **中文说明**    |
|---------|-----------------|-------------------|-------------|
| **apid** | **BIGINT**      | **主键**            | **申请记录 ID** |
| **oid** | **BIGINT**      | **→ orders(oid)** | **申请加入的订单** |
| **uid** | **BIGINT**      | **→ users(uid)**  | **申请人**     |
...
| **字段**            | **类型**     | **主外键**                      | **中文说明**             |
|-------------------| ------------------ |------------------------------|----------------------|
| **mid**           | **BIGINT**   | **主键**                       | **交流信息 ID**          |
| **oid**           | **BIGINT**   | **外键 → orders(oid)**         | **所属订单**             |
| **sender_uid**    | **BIGINT**   | **外键 → users(uid)**          | **提问或回复的人**          |
| **receiver_uid**  | **BIGINT**   | **外键 → users(uid)**          | **接收消息的人（一般是订单作者）**  |
| **reference_mid** | **BIGINT**   | **外键 → order_messages(mid)** | **引用的消息ID**          |
...
| **字段**           | **类型**     | **主外键**              | **中文说明**          |
|------------------| ------------------ |----------------------| --------------------------- |
| **acid**         | **BIGINT**   | **主键**               | **接受记录 ID**       |
| **oid**          | **BIGINT**   | **外键 → orders(oid)** | **被接受的订单 ID**   |
| **accepter_uid** | **BIGINT**   | **外键 → users(uid)**  | **接受订单的用户 ID** |
...
| **字段**         | **类型**            | **主外键**               | **中文说明**                                         |
|----------------|-------------------| ------------------------------ | ---------------------------------------------------------- |
| **pid**        | **BIGINT**        | **主键**                 | **内容唯一 ID（动态或评论）**                        |
| **uid**        | **BIGINT**        | **→ users(id)**         | **发布者**                                           |
| **parent_pid** | **BIGINT**        | **→ content_nodes(id)** | **父节点 ID（null 表示动态，非 null 表示评论回复）** |
| **oid**        | **BIGINT**        | **→ orders(id)，可空**  | **动态引用的订单 ID，可空**                          |
| **type**       | **PostType**      |                                | **内容类型**                                         |
| **content**    | **TEXT**          |                                | **文本内容**                                         |
| **has_media**  | **MediaType**     |                                | **是否为纯文本**                           |
...
| **字段**         | **类型**         | **主外键**          | **中文说明**     |
|----------------| ---------------------- |------------------| ---------------------- |
| **pmid**       | **BIGINT**       | **主键**           | **媒体文件 ID**  |
| **pid**        | **BIGINT**       | **→ posts(pid)** | **所属内容节点** |
...
| **字段**                      | **类型**     | **主外键**               | **中文说明**        |
|-----------------------------| ------------------ |-----------------------| ------------------------- |
| **plid**                    | **BIGINT**   | **主键**                | **点赞记录 ID**     |
| **pid**                     | **BIGINT**   | **→ posts(pid)**      | **所点赞的内容 ID** |
| **uid**                     | **BIGINT**   | **→ users(uid)**      | **点赞用户**        |
